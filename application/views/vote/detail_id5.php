<style>
    ...
    #vote-brief {position:relative;margin-top:-86px;z-index:2;}
        /* 报名按钮（若有） */
        #vote-signup {background-color:#510e70;width:300px;height:86px;line-height:86px;border-radius:43px;box-shadow:0 0 12px #c4c3d5 inset;}
    ...
    #vote-options {}
        /* 候选项单项 */
        .vote-option {background-color:rgba(255,255,255,.3);border:none;}
            .option-figure {background:transparent;}

            .option-brief>*, .option-actions>* {color:#fff;}
            .option-brief {line-height:28px;}
            .option-brief>*:not(:last-child) {margin-bottom:20px;}
                .option-name {}
                .ballot-count {}
                    .ballot-count span {color:#fff;}
                .option-actions a {color:#fff;background:#40035c;border-radius:30px;}
                    .option-actions a:last-child:not(:first-child) {color:#40035c;}
...
<div id=content>
    ...
    <div id=vote-info>
        ...
        <?php
        // 活动特有DOM
        require_once VIEWPATH.'vote/extra_id'.$item['vote_id'].'.php';
        ?>
    </div>
...
<script>
    var item = <?php echo json_encode($item) ?>; // 投票活动详情
    var tags = <?php echo json_encode($tags) ?>; // 投票候选项标签
    var options = <?php echo json_encode($options) ?>; // 投票候选项
