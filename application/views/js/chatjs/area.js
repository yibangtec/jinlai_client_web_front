...
$(".content .dqld_div").remove();
	...
	newStr.push("</ul></div>");
	$(".content").append(newStr.join(""));
}

function getCityBuy(val){
	var province=eval(proStr);
	var city=eval(province[val].ITEMS);
	console.log(province[val].NAME);
	$('#province').html(province[val].NAME);
	var newStr=new Array();
...
function getallArea(val,val1,val2){
	...
	console.log(area[val2].NAME)
	$('#area').html(area[val2].NAME);
